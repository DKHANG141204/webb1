...
function displayBill() {
  ...
  let productContainers = document.querySelectorAll('.pricing'); // Select all pricing containers
  if (cartItems && productContainers) {
    for (let productContainer of productContainers) { // Loop through each container
      productContainer.innerHTML = '';
      Object.values(cartItems).map(item => { // Loop through each cart item
        productContainer.innerHTML += `
          <div class="row">
            <div class="col-md-9 pull-left">
              <span id="name">${item.inCart}x ${item.name}</span>
            </div>
            <div class="col-md-3 pull-right">
              <span id="price">${item.inCart*item.price}.000.000₫</span>
            </div>
          </div>
        `;
      });
